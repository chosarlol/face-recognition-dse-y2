"""
capture.py  —  UPGRADED (drop-in replacement)

Changes vs original:
  - Uses DNN SSD detector instead of Haar Cascade (better accuracy)
  - --name argument: saves images per person into split_dataset/train/ and val/
  - Auto 80/20 train/val split at the end
  - Progress bar, 0.5s capture interval (faster than original 1s)
  - Images resized to 160x160 (FaceNet-compatible)

Usage:
    python capture.py --name Josar
    python capture.py --name Borom --camera 1
"""
...
def capture_dataset(name, camera_idx):
    ...
    while count < TOTAL_IMAGES:
        ...
        cv2.imshow(f"Capture: {name}", disp)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Face Dataset Capture")
